...
the skill curator (`skills/AGENTS.md`). Cron agents load built-in MEMORY.md/USER.md but suppress
external memory provider sync by default (`cron.sync_memory: false`); per-job `sync_memory`
overrides. See `cron/AGENTS.md`.
